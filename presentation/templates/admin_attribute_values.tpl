...
<form method="post" 
	action="{$obj->mLinkToAttributeValuesAdming}">
	<h3>
		Editting values for attribute: {$obj->mAttributeName}[
		<a href="{$obj->mLinkToAttributeAdmin}">back to attributes ...</a>]
	</h3>
	{if $obj->mErrorMessage} <p class="error">{$obj->mErrorMessage}</p>{/if}
	{if $obj->mAttributeValuesCount eq 0}
		<p class="no-items-found">There are no value for this attribute!</p>
	{else}
