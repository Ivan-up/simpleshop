...
<form method="get" action="{$obj->mLinkToAdmin}">
	...
	<p>
		...
		<input type="text" name="recordCount" value="{$obj->mRecordCount}" />
	...
	<p>
		...
		<input type="text" name="startDate" value="{$obj->mStartDate}" />
		<font class="bold-text">and</font>
		<input type="text" name="endDate" value="{$obj->mEndDate}" />
		<input type="submit" name="submitBetweenDates" value="Go" />
	</p>
	<p>
		<font class="bold-text">Show orders by status </font>
		{html_options name="status" options=$obj->mOrdersStatusOptions
...
<table class="tss-table">
	...
	<tr>
		...
		<td>{$obj->mOrdersStatusOptions[$status]}</td>
